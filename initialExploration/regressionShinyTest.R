...
# Shiny -------------------------------------------------------------------
# source("loadData.R")
setwd("/Users/cassandrabayer/DataScience/visionarieskaggle/flaredown/visionarieskaggle/shiny")
getwd()
...
ui <- fluidPage(
  ...
  sidebarPanel(
    ...
    selectInput(inputId = "DepVar", label = "Dependent Variables", multiple = FALSE, choices = list("age", "sex", "tookTreatment", "counterBetweenFlares", "counterDuringFlares", "winter", "summer", "spring", "fall")),
    p("Select the inputs for the Independent Variable"),
    selectInput(inputId = "IndVar", label = "Independent Variables", multiple = FALSE, choices = list( "avgPain"))
  ),
  mainPanel(
